...
class DataHandler():
    ...
    def _plot_gradient_hist(self, top_arr, num_bin, ax, 
        ...
        if data_format == 'line':
            ...
            top_hist_freq, _ = np.histogram(top_arr, bins=x_axis)
            if bottom_arr is None:
                bottom_hist_freq = np.zeros(num_bin + 1)
            else:
                bottom_hist_freq, _ = np.histogram(bottom_arr, bins=x_axis)
    ...
    def plot_dist_classifier_scores(self, df=None, num_bin=100, fig_name=None):
        if df is None:
            df = self.observed_df

        ax = self._prepare_canvas()
    ...
    def plot_stacked_frequency(self, df=None, num_bin=100, fig_name=None):
        if df is None:
            df = self.observed_df
    ...
    def plot_class_conditional_density(self, df=None, num_bin=100, fig_name=None):
        if df is None:
            df = self.observed_df
        ...
        self._plot_gradient_hist(df[df['GT'] == True]['C(X)'].values, 
                                 num_bin=num_bin, ax=ax, color=self.positive_color)
        self._plot_gradient_hist(df[df['GT'] == False]['C(X)'].values, 
                                 num_bin=num_bin, ax=ax, color=self.negative_color)
    ...
    def plot_calibration_curve(self, df=None, num_bin=100, show_diagonal=False,
                               method='perfect calibration', fig_name=None):
        if df is None:
            df = self.observed_df
        ax = self._prepare_canvas()
        ...
        if method=='perfect calibration':
            ...
            self._plot_gradient_hist(df[df['GT'] == True]['C(X)'].values, 
                                     num_bin=num_bin, ax=ax, color=self.positive_color,
                                     bottom_arr=df['C(X)'].values, color2=self.negative_color, 
                                     relation='relative')
        ...
        ax.set_xlabel('$C(X)$', fontsize=16)
        ax.set_ylabel('$P(GT=1|C(X))$', fontsize=16)
    ...
    def plot_dist_gt_labels(self, df=None, set_pp=False, p_p=1, fig_name=None):
        if set_pp:
            p_n = 1 - p_p
        else:
            if df is None:
        ...
        ax.set_xticks([0, 1])
        ax.set_xticklabels(['GT=0', 'GT=1'], fontsize=16)
        ax.set_ylabel('$P(GT)$', fontsize=16)
        ax.set_yticks([0, 0.5, 1])
        ax.set_ylim([0, 1])
...
class CalibrationCurve():
    ...
    A calibration curve.
    """
    def __init__(self, uncalibrated_cx, calibrated_cx):
        self.x = np.array(uncalibrated_cx)
        self.y = np.array(calibrated_cx)

    def get_calibrated_prob(self, cx):
        nearest_idx = (np.abs(self.x - cx)).argmin()
        return self.y[nearest_idx]
...
class ExtrinsicEstimator():
    """
    A class for extrinsic estimators.
    """
    def __init__(self, uncalibrated_cx=None):
        self.uncalibrated_cx = None
        self.calibration_curve = None

    def set_cx_input(self, uncalibrated_cx):
        self.uncalibrated_cx = np.array(uncalibrated_cx)

    def set_calibration_curve(self, calibration_curve: CalibrationCurve):
        self.calibration_curve = calibration_curve

    def prob_classify_count(self):
        calibrated_cx = [self.calibration_curve.calibrate(cx) for cx in self.uncalibrated_cx]
        return np.mean(calibrated_cx)
